var logger = require('pomelo-logger').getLogger('bearcat-tgame', 'Mjutils');
...
Mjutils.prototype.checkTingPai = function (seatData,begin,end){
	for(var i = begin; i < end; ++i){
		...
		//逐个判定手上的牌
		var ret = this.checkCanHu(seatData);
		if(ret){
			//平胡 0番
			seatData.tingMap[i] = {
				pattern:"normal",
...
Mjutils.prototype.matchSingle = function (seatData,selected){
	//分开匹配 A-2,A-1,A
	var matched = true;
	var v = selected % 9;
	if(v < 2){
	...
	if(matched){
		...
		var ret = this.checkSingle(seatData);
		seatData.countMap[selected - 2] ++;
		seatData.countMap[selected - 1] ++;
		seatData.countMap[selected] ++;
		if(ret == true){
...
Mjutils.prototype.checkSingle = function (seatData){
...
Mjutils.prototype.checkCanHu = function (seatData){
...
Mjutils.prototype.getMJType = function(pai){
      if(id >= 0 && id < 9){
          return 0;
      }
      else if(id >= 9 && id < 18){
